...
def insert(request):
	...
	try:
		...
		ob.picname = filename #此处图片已被重新命名
		ob.state = 1
		...
		print(err)
		context={"info":"添加失败,请将信息填写完整！"}
		os.remove("./static/goods/m_"+filename) #执行图片删除  
		os.remove("./static/goods/s_"+filename) #执行图片删除  
		os.remove("./static/goods/"+filename) #执行图片删除
...
def edit(request,gid):
	try:
		...
		for oc in list:
			oc.pname = '. . .'*(oc.path.count(',')-1)
		# 放置信息加载模板
		context = {"goodslist":list,'goods':ob}
...
def update(request,gid):
	...
	try:
		b = False #和下面的b=True配合，判断信息有没有成功上传，如果没有成功完整上传要删除新上传成功的部分照片
		oldpicname = request.POST['oldpicname']
		myfile = request.FILES.get("pic",None)
		
		if not myfile:
			picname = oldpicname #如果没有新照片成功上传，将老照片的名字赋给picname之后下文保存	
		else:
			filename = str(time.time())+"."+myfile.name.split('.').pop()
